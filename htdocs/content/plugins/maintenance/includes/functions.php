...
function mtnc_get_fonts_subsets($title, $id, $name, $value)
{
  ...
  $curr_font = esc_attr( isset($mt_option['body_font_family']) ? $mt_option['body_font_family'] : '' );
  $vars      = 'subsets';
...
?>
  <table class="form-table">
    <tbody>
      ...
      mtnc_generate_image_filed(__('Logo', 'maintenance'), 'logo', 'logo', isset($mt_option['logo']) ? (int) $mt_option['logo'] : '', 'boxes box-logo', __('Upload Logo', 'maintenance'), 'upload_logo upload_btn button');
      mtnc_generate_image_filed(__('Retina Logo (optional)', 'maintenance'), 'retina_logo', 'retina_logo', isset($mt_option['retina_logo']) ? (int) $mt_option['retina_logo'] : '', 'boxes box-logo', __('Upload Retina Logo', 'maintenance'), 'upload_logo upload_btn button');
      ...
      mtnc_generate_number_field(__('Set Blur Intensity', 'maintenance'), 'blur_intensity', 'blur_intensity', isset($mt_option['blur_intensity']) ? (int) $mt_option['blur_intensity'] : '' );
...
function mtnc_add_css_fields()
{
  ...
  mtnc_generate_textarea_filed(__('CSS Code', 'maintenance'), 'custom_css', 'custom_css', wp_kses_stripslashes( isset($mt_option['custom_css']) ? $mt_option['custom_css'] : '' ));
...
{
  $mt_option = mtnc_get_plugin_options(true);
  mtnc_generate_image_filed(__('Background Image', 'maintenance'), 'body_bg', 'body_bg', esc_attr( isset($mt_option['body_bg']) ? $mt_option['body_bg'] : '' ), 'boxes box-bg', __('Upload Background', 'maintenance'), 'upload_background upload_btn button');
}
...
{
  $mt_option = mtnc_get_plugin_options(true);
  mtnc_get_color_field(__('Background Color', 'maintenance'), 'body_bg_color', 'body_bg_color', esc_attr( isset($mt_option['body_bg_color']) ? strip_tags($mt_option['body_bg_color']) : ''), '#111111');
  mtnc_get_color_field(__('Font Color', 'maintenance'), 'font_color', 'font_color', esc_attr( isset($mt_option['font_color']) ? strip_tags($mt_option['font_color']) : ''), '#ffffff');
...
{
  $mt_option = mtnc_get_plugin_options(true);
  mtnc_wp_kses(mtnc_get_fonts_field(__('Font Family', 'maintenance'), 'body_font_family', 'body_font_family', esc_html( isset($mt_option['body_font_family']) ? $mt_option['body_font_family'] : '' )));
  $subset = '';
