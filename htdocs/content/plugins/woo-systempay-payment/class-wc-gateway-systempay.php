...
class WC_Gateway_Systempay extends WC_Payment_Gateway
{
     ...
     **/
    public function systempay_manage_notify_response($systempay_response, $from_server_rest = false, $update_on_failure = true)
    {
        global $woocommerce, $systempay_plugin_features;
        ...
        if (SystempayRestTools::checkResponse($_POST)) {
            ...
            if (method_exists($order, 'set_payment_method')) {
                $order->set_payment_method($systempay_std);
            } else {
                $order->payment_method = $systempay_std->id;
                $order->payment_method_title = $systempay_std->get_title();
            }
