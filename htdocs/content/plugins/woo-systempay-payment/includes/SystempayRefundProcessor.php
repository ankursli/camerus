...
class SystempayRefundProcessor implements RefundProcessor
{
    ...
    public function doOnError($errorCode, $message)
    {
        ...
        set_transient('systempay_online_refund_result', $msg);
    }
    ...
    public function doOnSuccess($operationResponse, $operationType)
    {
        if ($operationType == 'frac_update') {
            $this->doOnError(-1, sprintf($this->translate('Refund of split payment is not supported. Please, consider making necessary changes in %1$s Back Office.'), 'Systempay'));
        } else {
