...
class WC_Order_Export_Admin {
	...
	public function ajax_gate() {
		...
		if ( ! isset( $this->tabs[ $tab ] ) ) {
			...
			if ( ! method_exists( $ajax_handler, $method ) ) {
				die( sprintf( __( 'Unknown AJAX method %s', 'woocommerce-order-export' ), esc_html($method)) );
			}
		...
		if ( ! method_exists( $this->tabs[ $tab ], $method ) ) {
			die( sprintf( __( 'Unknown tab method %s', 'woocommerce-order-export' ), esc_html($method)) );
		}
	...
	function export_orders_bulk_action( $actions ) {
		...
		if ( ! empty( $settings['format'] ) ) {
			$new_actions['woe_export_selected_orders'] = sprintf( __( 'Export as %s', 'woocommerce-order-export' ),
				$settings['format'] );
		}

		// mark/unmark
		if ( $this->settings['show_export_actions_in_bulk'] ) {
			$new_actions['woe_mark_exported']   = __( 'Mark exported', 'woocommerce-order-export' );
			$new_actions['woe_unmark_exported'] = __( 'Unmark exported', 'woocommerce-order-export' );
		}
